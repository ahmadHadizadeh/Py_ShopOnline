...
def merge_guest_cart_to_user(request):
    ...
    user_cart, _ = Cart.objects.get_or_create(user=request.user, status=Cart.STATUS_ACTIVE)
    guest_cart = (
    ...
    guest_items = guest_cart.items.select_for_update().select_related("product", "variant")
    for guest_item in guest_items:
        item_filter = {"product_id": guest_item.product_id, "variant_id": guest_item.variant_id}
        existing_item = (
        ...
        if existing_item:
            merged_quantity = min(
                ...
                guest_item.product.stock,
            )
            if merged_quantity >= 1:
                existing_item.quantity = merged_quantity
                existing_item.status = CartItem.STATUS_ACTIVE
                existing_item.unit_price_snapshot = _unit_price(existing_item.product, existing_item.variant)
            ...
            guest_item.quantity = min(guest_item.quantity, max(1, guest_item.product.stock))
            guest_item.unit_price_snapshot = _unit_price(guest_item.product, guest_item.variant)
